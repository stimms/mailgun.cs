...
namespace MailGun
{
    ...
    public abstract class MailgunResource<T> where T : MailgunResource<T>, new()
    {
        ...
        public void Delete()
        {
            using (Utilities.SendRequest(Utilities.OpenRequest(resourceUrl, "DELETE")))
            {
            }
        }
        ...
        public static List<T> Find()
        {
            ...
            // dirty but simple
            HttpWebRequest wr = Utilities.OpenRequest(collectionUrl, "GET");
            using (HttpWebResponse r = Utilities.SendRequest(wr))
            {
                if (isResponseXml(r))
                {
        ...
        {
            HttpWebRequest wr = Utilities.OpenRequest(url, method);
            wr.ContentType = "text/xml";
            using (Stream rs = Utilities.GetRequestStream(wr))
                writeXml(rs);
        ...
        {
            get { return Utilities.ApiUrl + collectionName + "/" + Id + ".xml"; }
        }

        protected string collectionUrl
        {
            get { return Utilities.ApiUrl + collectionName + ".xml"; }
        }
    ...
    public abstract class MailgunResourceUpsertable<T> : MailgunResource<T> where T : MailgunResourceUpsertable<T>, new()
    {
        ...
        public void Upsert()
        {
            update(Utilities.ApiUrl + collectionName + "/upsert.xml", "POST", false);
        }
    }
    ...
    public class Mailbox : MailgunResourceUpsertable<Mailbox>
    {
        ...
        {
            HttpWebRequest wr = Utilities.OpenRequest(Utilities.ApiUrl + "mailboxes.txt", "POST");
            ...
            using (Utilities.SendRequest(wr))
            {
            }
        }
    }
    ...
    public class MailgunMessage
    {
        ...
        public static void SendText(string sender, string recipients, string subject, string text, string servername, MailgunMessage.Options options)
        {
            ...
            HttpWebRequest wr = Utilities.OpenRequest(messagesUrl("txt", servername), "POST");
        ...
        {
            HttpWebRequest wr = Utilities.OpenRequest(messagesUrl("mime", servername), "POST");
        ...
        {
            return Utilities.ApiUrl + "messages." + format + "?servername=" + servername;
        }
